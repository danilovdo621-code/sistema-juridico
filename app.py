...
if not st.session_state['logged_in']:
    ...
    st.markdown("""
        ...
        /* Imagem Fixa na Esquerda (60% da tela) */
        .split-bg {
        ...
        /* Sombra sobre a imagem para dar contraste */
        .split-bg::after {
        ...
        .login-subtitle {
            ...
            text-align: center; font-size: 0.9rem; margin-bottom: 2.5rem;
        }

        /* FORÇA O ALINHAMENTO VERTICAL NO CENTRO (A MÁGICA) */
        div[data-testid="column"]:nth-of-type(2) > div {
            display: flex;
            flex-direction: column;
            justify-content: center;
            height: 95vh; /* Ocupa quase toda a altura */
        ...
        @media only screen and (max-width: 768px) {
            .split-bg { display: none !important; } /* Some a imagem */

            /* Remove o alinhamento forçado do desktop */
            div[data-testid="column"]:nth-of-type(2) > div {
        ...
        div[data-testid="stTextInput"] input:focus { 
            border-color: #1e40af; background-color: #ffffff;
        ...
        div[data-testid="stButton"] button:hover { 
            background-color: #1e3a8a; transform: translateY(-1px);
    ...
    # Layout de Colunas: 1.5 (Imagem) vs 1 (Login)
    # Isso dá uma proporção de 60% / 40%
    c1, c2 = st.columns([1.5, 1])

    with c1:
        st.write("") # Coluna da imagem (vazia, pois o CSS cuida do fundo)

    with c2:
        # O CSS acima vai centralizar tudo o que estiver aqui dentro verticalmente
        st.markdown('<div class="login-title">⚖️ Jurídico Pro</div>', unsafe_allow_html=True)
        st.markdown('<div class="login-subtitle">Portal de Acesso ao Cliente</div>', unsafe_allow_html=True)
        ...
        st.markdown("<div style='text-align: center; color: #94a3b8; font-size: 11px; margin-top: 40px;'>© 2025 Advocacia Digital • Segurança SSL</div>", unsafe_allow_html=True)
