...
# ==============================================================================
# 2. CONEXÃO COM GOOGLE SHEETS (HÍBRIDA: NUVEM E PC)
# ==============================================================================
try:
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/spreadsheets"]

    # Tenta ler do Cofre Digital (Streamlit Cloud)
...
if not st.session_state['logged_in']:
    ...
    st.markdown("""
        ...
        /* ESTILO MOBILE (CELULAR) */
        @media only screen and (max-width: 768px) {
            .split-bg {
                display: none; /* Esconde a imagem lateral no celular */
            }
            .stApp {
                background-color: #f8fafc; /* Fundo claro para o celular */
            }
        }

        div[data-testid="stTextInput"] input { border: 1px solid #cbd5e1; border-radius: 6px; padding: 10px 12px; font-family: 'Inter', sans-serif; }
        div[data-testid="stTextInput"] input:focus { border-color: #1e40af; box-shadow: 0 0 0 2px rgba(30, 64, 175, 0.2); }
        div[data-testid="stButton"] button { width: 100%; background-color: #1e40af; color: white; border: none; padding: 12px; font-weight: 600; border-radius: 6px; transition: background 0.3s; }
        div[data-testid="stButton"] button:hover { background-color: #1e3a8a; }
        </style>
        <div class="split-bg"></div>
    """, unsafe_allow_html=True)

    col_vazia, col_login = st.columns([6, 4]) 

    with col_login:
        st.markdown("<br><br><br><br><br>", unsafe_allow_html=True)
        with st.container():
            st.markdown("<h1 style='text-align: center; color: #1e3a8a; font-family: Inter;'>⚖️ Jurídico Pro</h1>", unsafe_allow_html=True)
            st.markdown("<p style='text-align: center; color: #64748b; margin-bottom: 30px;'>Acesso Restrito</p>", unsafe_allow_html=True)
            ...
            if st.button("ENTRAR"):
                if CONNECTION_STATUS:
                    ...
                    st.error("O sistema não conseguiu conectar com a planilha.")

            st.markdown("<p style='text-align: center; font-size: 12px; color: #94a3b8; margin-top: 50px;'>© 2025 Sistema Jurídico.</p>", unsafe_allow_html=True)
